        ...
        <form method="POST" action="{{ route('admin.purchase_orders.update', $purchaseOrder->id) }}">
            ...
            <div class="py-4">
                ...
                <div id="products-container" class="border rounded p-4 mt-2">
                    ...
                    <div class="grid grid-cols-4 gap-4 mb-4 product-row" data-index="{{ $index }}">
                        <div>
                        ...
                        <div>
                            <x-admin.form.label for="products[{{ $index }}][price]" class="{{ $errors->has('products.' . $index . '.price') ? 'text-red-400' : '' }}">{{ __('Price') }}</x-admin.form.label>
                            <input id="products[{{ $index }}][price]" class="{{ $errors->has('products.' . $index . '.price') ? 'border-red-400' : '' }} input input-bordered" type="number" step="0.01" name="products[{{ $index }}][price]" value="{{ old('products.' . $index . '.price', $product->pivot->price) }}" />
                        </div>
                        <div>
